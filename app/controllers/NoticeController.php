...
class NoticeController extends BaseController {
	...
	public function NoticeList()
	{
	    ...
	    $noticeList = DB::connection('zenkr.hz')->select('SELECT * FROM `mbaa`.`story` ORDER BY sid DESC  LIMIT 50');
		return View::make('static.story', array('head_title' => '故事 - MBAA.cn' , 'storyList' => $noticeList));
	}
	
    public function NoticeShow($nid)
	{
	    $notice = DB::connection('zenkr.hz')->select('select * from notice where nid = ?', array($nid));
